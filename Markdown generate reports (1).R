...
yak_23 <-  read.csv("softball/10_06_2024 3_25_57 PM-Missouri State@Arkansas Softball 2024 (1).csv")

# rename the dataset and filter by the boomers most recent game 
yakker <- yak_23 %>%
  # FILTER
  filter(PitcherTeam == 'Arkansas softball 2024') %>%
...
yakker$TaggedPitchType <- factor(yakker$TaggedPitchType, levels = c('Riseball', 'Dropball', 'Fastball', 'Changeup', 'Curveball', 'Screwball')
)

# extract the date for the title of the report and change to different format
# yakker$Date <- as.Date(yakker$Date, format = "%m/%d/%Y")
# game_date <- unique(format(yakker$Date, "%b %d, %Y"))
...
for (pitcher in pitchers) {
  ...
  game_summary_table <- 
    ...
    dplyr:: mutate(TaggedPitchType = recode(TaggedPitchType, Riseball = "RB", Dropball="DB", 
                            Fastball = "FB", Changeup = "CH", Curveball = "CB", Screwball = "SB") ) %>%
  ...
  pitch_movement_plot <- 
    ...
    scale_color_manual(values = c('RB' = 'red', 'DB' = 'darkgreen', 'FB' = '#f47b20',  'CH'='cornflowerblue',
                                  'CB' = 'gold',  'SB'='violet')) +  
  ...
  plp_rhh <- 
    ggplot(data = pitcher_data %>% filter(BatterSide=='Right' )%>%
           dplyr:: mutate(TaggedPitchType = recode(TaggedPitchType, Riseball = "RB", Dropball="DB", 
                                                   Fastball = "FB", Changeup = "CH", Curveball = "CB", Screwball = "SB") ), 
  ...
  plp_lhh <- 
    ggplot(data = pitcher_data %>% filter(BatterSide=='Left' )%>%
             dplyr:: mutate(TaggedPitchType = recode(TaggedPitchType, Riseball = "RB", Dropball="DB", 
                                                     Fastball = "FB", Changeup = "CH", Curveball = "CB", Screwball = "SB"
             ) ), 
           aes(x = PlateLocSide, y = PlateLocHeight, color = TaggedPitchType)) +
  ...
  breakdown<-ggplot(bd %>%
    ...
    scale_fill_manual(values = c('RB' = 'red', 'DB' = 'darkgreen', 'FB' = '#f47b20',  'CH'='cornflowerblue',
                                 'CB' = 'gold',  'SB'='violet'))
  ...
  file_date <- gsub("/", "-", game_stats$Date[1])
  
  # Knit the R Markdown file to PDF
  rmarkdown::render(input = "softball/Scouting-Reports-P (1).Rmd",
                    output_file = paste0("softball/",file_date,pitcher, "report",".pdf"),
                    params = params)
}
